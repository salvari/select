...
double y(double x) 
{ 
  return f(x) + g(x) + h(x) + i(x) + j(x) + k(x) + m(x); 
}
...
//  grid(32,32); // fine grid
 ...
 // v_axis_masklabels(P(x_min, y_min), P(x_min, y_max), 4, P(-10,0), c);
  ...
  cropplot(i,x_min,x_max,h_size);
 
  cropplot(j,x_min,x_max,h_size);
  
  cropplot(k,x_min,x_max,h_size);
  
  cropplot(m,x_min,x_max,h_size);
  ...
  black();
  label(P(3.14,1.1),"$\\hbox{1st + 3rd + 5th + 7th + 9th + 11th + 13th}$");
