...
/**********************************************************
 ...
 Copyright 2005, Tony R. Kuphaldt, released under the 
 Creative Commons Attribution License (v 1.0).

 This means you may do almost anything with this work of 
 mine, so long as you give me proper credit.

Compile this C++ source file by invoking the ePiX 
utility, "elaps" from the command line like this:

    elaps filename.cpp

The result will be an Encapsulated PostScript file of the 
same name, except with the extension ".eps":
...
main() 
{
  ...
  label(P(6,3.4) , "$\\hbox{Whatever}$");
  label(P(6,-3.4) , "$\\hbox{Who cares?}$");
